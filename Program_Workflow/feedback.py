# feedback.py — dataset_creation/{audio|frames}/{real|fake} with preview → confirm flow
...
def run_feedback_interactive(
    ...
    *,
    ...
    # Pass already-loaded models to avoid reloading them (saves memory + time)
    a_model=None,
    a_meta=None,
    i_model=None,
    i_meta=None,
    ...
    """
    Preview → confirm → save with structure:
      dataset_creation/
        audio/  real/  fake/
        frames/ real/  fake/
    """
    ...
    if not vpath.exists():
        print(f"[FEEDBACK] Video not found: {vpath}")
        return
    if not _ffprobe_ok(vpath):
        print("[FEEDBACK] Input is damaged/unreadable. Skipping.")
        return
    ...
    if cfg.normalize_video:
        try:
            ...
            print(f"[FEEDBACK] Normalisation failed ({e}). Skipping.")
            return
    ...
    # FIX: always initialise temp_dir so the cleanup block never hits NameError
    ...
    # 3) Load models only if not passed in
    if a_model is None or a_meta is None:
        a_model, a_meta = load_audio_model()
    if i_model is None or i_meta is None:
    ...
    if audio_mp3 is not None and audio_mp3.exists():
        ...
        print(f"AUDIO -> {ra['label']} ({_pct(ra['prob'])})")
        ...
        if dists:
            ...
            print(f"AUDIO (clips avg) -> {pred_k} ({_pct(mean_dist[pred_k])})")
    ...
    if frames_dir is not None and frames_dir.exists():
        ...
        if imgs:
            ...
            for p in imgs:
                ...
                print(f"  {p.name} -> {ri['label']} ({_pct(ri['prob'])})")
                d = ri["dist"]
                if dist_sum is None:
                    dist_sum = {k: d[k] for k in d}
                else:
                    for k in d:
                        dist_sum[k] += d[k]
                n += 1
            if dist_sum and n:
                ...
                pred_k = max(mean_dist, key=lambda k: mean_dist[k])
                print(f"FRAMES (mean) -> {pred_k} ({_pct(mean_dist[pred_k])})")
        else:
            print("FRAMES -> (no images found)")
    else:
        print("FRAMES -> (none)")

    # 5) Ask if we should add to dataset at all
    print("\n=== Add this sample to the dataset? ===")
    if not _yesno("Add to dataset"):
        print("[FEEDBACK] Discarded by user.")
        # FIX: safe cleanup even when user says no
        if temp_dir is not None and temp_dir.exists():
            shutil.rmtree(temp_dir)
        return

    # 6) Ask labels for audio & frames (independent)
    audio_label = None
    frames_label = None
    if (audio_mp3 is not None) or (audio_clips_dir is not None and audio_clips_dir.exists()):
        audio_label = _ask_label("AUDIO")
    if frames_dir is not None and frames_dir.exists():
        frames_label = _ask_label("FRAMES")
    ...
    print("\n[FEEDBACK] Saved:")
    if saved_audio:
        print(f"  AUDIO  -> {dst_audio_dir} (+{len(saved_audio)} files)")
    if saved_frames:
        print(f"  FRAMES -> {dst_frames_dir} (+{len(saved_frames)} files)")
    ...
    # FIX: guard against NameError — temp_dir is always defined (None if precomputed)
    if temp_dir is not None and temp_dir.exists():
        shutil.rmtree(temp_dir)
